...
samp %>%
  summarise(x_bar=mean(price), s_dev=sd(price))
...
ci <- ames %>%
        rep_sample_n(size = n, reps = 50, replace = TRUE) %>%
        summarise(lower = mean(area) - z_start_99 * (sd(area) / sqrt(n)),
                  upper = mean(area) + z_start_99 * (sd(area) / sqrt(n)))
```
